...
def analyzeText(text, language, threshold, tt, ud):
    ...
    topics = {'person': [], 'organization': [], 'location': [], 'product': [], 'id': [], 'event': [], 'other': [], 'quantity': []}

    try:
        # We are going to make a request to the Topics Extraction API
        request  = meaningcloud.TopicsRequest(license_key, txt=text, lang=language, topicType=tt, server=server, otherparams={'ud': ud})
        ...
        if response.isSuccessful():
                ...
                if entity_list:
                    for entity in entity_list:
                        if int(response.getTopicRelevance(entity)) >= threshold:
                            first_node = response.getTypeFirstNode(response.getOntoType(entity)).lower()
                            form = str(response.getTopicForm(entity))
                            insertInList(topics.get('other'), form) if topics.get(first_node) is None else insertInList(topics.get(first_node), form)
            if 'c' in tt:
                concept_list = response.getConcepts()           
                if concept_list:
                    for concept in concept_list:
                        if int(response.getTopicRelevance(concept)) >= threshold:
                            first_node = response.getTypeFirstNode(response.getOntoType(concept)).lower()
                            form = str(response.getTopicForm(concept))
                            insertInList(topics.get('other'), form) if topics.get(first_node) is None else insertInList(topics.get(first_node), form)
            if 'm' in tt:
                money_expression_list = response.getMoneyExpressions()
                if money_expression_list:                    
                    [insertInList(topics.get('quantity'), str(response.getTopicForm(money))) for money in money_expression_list]
            if 'n' in tt:
                quantity_expression_list = response.getQuantityExpressions()
                if quantity_expression_list:
                    [insertInList(topics.get('quantity'), str(response.getTopicForm(quantity))) for quantity in quantity_expression_list]
        else:
            if isBlockingErrorType(response.getStatusCode()):
                ...
                print("Oops! The request to Topics Extraction for text #" + str(index_count) + " was not succesful: (" + response.getStatusCode() + ') ' + response.getStatusMsg())
                topics = {
                    'person': 'ERROR (' + response.getStatusCode() + '): ' + response.getStatusMsg(),
                    'organization': "", 'location': "", 'product': "", 'id': "", 'event': "", 'other': "", 'quantity': ""
                }
...
print("Starting analysis for %d texts to extract '%s' in '%s'... " % (len(input_df), tt, language))
...
# main analysis
index_count = 1
label_list =  ['Person', 'Organization', 'Location', 'Product', 'ID', 'Event', 'Other', 'Quantity']
output_df[label_list] = input_df[text_column].apply(analyzeText, language=language, threshold=threshold, tt=tt, ud=user_dictionary)
