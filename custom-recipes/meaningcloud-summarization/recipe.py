...
def analyzeText(text):
    ...
    try:
        ...
        request = meaningcloud.SummarizationRequest(license_key, sentences=sentences, txt=text, server=server)
        ...
        if response.isSuccessful():
            ...
            if isBlockingErrorType(response.getStatusCode()):
                ...
                print("Oops! The request to Summarization for text #" + str(index_count) + " was not succesful: (" + response.getStatusCode() + ') ' + response.getStatusMsg())
                summary = 'ERROR (' + response.getStatusCode() + '): ' + response.getStatusMsg()
