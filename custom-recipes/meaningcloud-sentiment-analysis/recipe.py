...
def analyzeText(text, lang):
    ...
    try:
        ...
        request = meaningcloud.SentimentRequest(license_key, lang=lang, txt=text, server=server)
        setRequestSource(request)
        response = meaningcloud.SentimentResponse(request.sendReq())
        if response.isSuccessful():
            polarity = response.getGlobalScoreTag()        
        else:
            if isBlockingErrorType(response.getStatusCode()):
                ...
                print("Oops! The request to Sentiment Analysis for text #" + str(index_count) + " was not succesful: (" + response.getStatusCode() + ') ' + response.getStatusMsg())
                polarity = 'ERROR (' + response.getStatusCode() + '): ' + response.getStatusMsg()
